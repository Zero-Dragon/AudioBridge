...
public sealed partial class MainWindow : Window
{
    ...
    private readonly Dictionary<int, string> clockSourceNames = new();
    ...
    {
        var iconPath = Path.Combine(AppContext.BaseDirectory, "Assets", "TickByTick.ico");
    ...
    private void InitializeNative()
    {
        try
        {
            var result = TickByTickNative.TBT_Initialize();
            nativeReady = result == 0;
            if (!nativeReady)
            {
                AppendLog($"Native init failed: {TickByTickNative.LastError()}");
                return;
            }

            RefreshDevices();
            _ = RefreshClockSourcesAsync();
            TickByTickNative.TBT_SetPrebufferMs((int)PrebufferBox.Value);
            TickByTickNative.TBT_SetMaxBufferAdvanceMs((int)MaxBufferAdvanceBox.Value);
            pollTimer.Start();
            AppendLog("Tick By Tick ready.");
        }
        catch (DllNotFoundException ex)
        {
            AppendLog($"TickByTickCore.dll not found: {ex.Message}");
        }
        catch (Exception ex)
        {
    ...
    private void RefreshDevices()
    {
        ...
        devices.Clear();
        var result = TickByTickNative.TBT_RefreshDevices();
        if (result != 0)
        {
            AppendLog($"Device refresh failed: {TickByTickNative.LastError()}");
            return;
        }

        var count = TickByTickNative.TBT_GetDeviceCount();
        var defaultIndex = TickByTickNative.TBT_GetDefaultDeviceIndex();
        for (var i = 0; i < count; i++)
        {
            var id = TickByTickNative.ReadString((buffer, chars) =>
                TickByTickNative.TBT_GetDeviceId(i, buffer, chars));
            var name = TickByTickNative.ReadString((buffer, chars) =>
                TickByTickNative.TBT_GetDeviceName(i, buffer, chars));
        ...
        var outputResult = TickByTickNative.TBT_SetOutputDevice(settings.SelectedAsioDeviceId);
        if (outputResult != 0)
        {
            AppendLog($"ASIO output selection failed: {TickByTickNative.LastError()}");
        }
        PersistSettings("Could not remember the selected ASIO output");
    }

    private async Task RefreshClockSourcesAsync()
    {
        var generation = ++clockSourceRefreshGeneration;
        var deviceId = (DeviceComboBox.SelectedItem as AudioDeviceView)?.Id;

        clockSourceNames.Clear();
        ...
        await clockSourceRefreshLock.WaitAsync();
        try
        {
            if (generation != clockSourceRefreshGeneration || !nativeReady)
            {
                return;
            }
            ...
            {
                var result = TickByTickNative.TBT_RefreshClockSources(deviceId);
                var error = result == 0 ? string.Empty : TickByTickNative.LastError();
                var loadedSources = new List<(int Index, string Name, bool IsCurrent)>();
                if (result == 0)
                {
                    var count = TickByTickNative.TBT_GetClockSourceCount();
                    for (var position = 0; position < count; position++)
                    {
                        if (TickByTickNative.TBT_GetClockSourceInfo(position, out var info) != 0)
                        {
                            continue;
                        }
                        var name = TickByTickNative.ReadString((buffer, chars) =>
                            TickByTickNative.TBT_GetClockSourceName(position, buffer, chars));
                        loadedSources.Add((
                            info.Index,
                            string.IsNullOrWhiteSpace(name)
                                ? $"Clock source {info.Index}"
                                : name,
                            info.IsCurrent != 0));
                    }
                }
                return (Result: result, Error: error, Sources: loadedSources);
            });
            ...
            {
                clockSourceNames[source.Index] = source.Name;
            }
            ...
            var selectResult = TickByTickNative.TBT_SetClockSource(AutomaticClockSourceIndex);
            if (selectResult != 0)
            {
                AppendLog($"Could not keep the ASIO driver current clock source: {TickByTickNative.LastError()}");
            }
        }
        catch (Exception ex)
        {
    ...
    private void OpenButton_Click(object sender, RoutedEventArgs e)
    {
        ...
        var advanceResult = TickByTickNative.TBT_SetMaxBufferAdvanceMs(maxBufferAdvanceMs);
        if (advanceResult != 0)
        {
            var error = TickByTickNative.LastError();
            LastErrorText.Text = error;
            AppendLog($"Max buffer advance update failed: {error}");
            return;
        }
        var clockResult = TickByTickNative.TBT_SetClockSource(AutomaticClockSourceIndex);
        if (clockResult != 0)
        {
            var error = TickByTickNative.LastError();
            AppendLog($"Could not keep the ASIO driver current clock source: {error}");
        }
        var result = TickByTickNative.TBT_StartTargetWithOptions3(
            exePath,
            null,
            deviceId,
            prebufferMs,
    ...
    private void DeviceComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        var result = TickByTickNative.TBT_SetOutputDevice(deviceId);
        if (result != 0)
        {
            AppendLog($"ASIO output switch failed: {TickByTickNative.LastError()}");
            return;
        }
        _ = RefreshClockSourcesAsync();
    }
    ...
    private void PrebufferBox_ValueChanged(NumberBox sender, NumberBoxValueChangedEventArgs args)
    {
        ...
        var result = TickByTickNative.TBT_SetPrebufferMs(ReadPrebufferMs());
        if (result != 0)
        {
            AppendLog($"Prebuffer update failed: {TickByTickNative.LastError()}");
        }
        ...
        NumberBoxValueChangedEventArgs args)
    {
        ...
        var result = TickByTickNative.TBT_SetMaxBufferAdvanceMs(ReadMaxBufferAdvanceMs());
        if (result != 0)
        {
            AppendLog($"Max buffer advance update failed: {TickByTickNative.LastError()}");
        }
    ...
    private void PidListView_SelectionChanged(object sender, SelectionChangedEventArgs e)
    {
        ...
        var result = TickByTickNative.TBT_SelectAudioPid(selected.Pid);
        if (result != 0)
        {
            AppendLog($"PID selection failed: {TickByTickNative.LastError()}");
        }
    }
    ...
    private void PollNativeState()
    {
        ...
        if (TickByTickNative.TBT_GetStatus(out var status) == 0)
        {
            if (bridgeActive && status.Running == 0)
            {
                bridgeActive = false;
                TickByTickNative.TBT_Stop();
                AppendLog("Target process exited; bridge stopped.");
                TickByTickNative.TBT_GetStatus(out status);
            }
            UpdateStatus(status);
    ...
    private void UpdateStatus(TickByTickNative.Status status)
    {
        ...
        WasapiBufferText.Text = $"{status.WasapiBufferMs} ms";
        ToolTipService.SetToolTip(
            WasapiBufferText,
            status.WasapiBufferFrames > 0
                ? $"{status.WasapiBufferFrames:N0} frames; effective timeline {status.EffectiveTimelineMs} ms"
                : "Waiting for a fake WASAPI stream");
        ...
        {
            var pipelineError = TickByTickNative.LastError();
    ...
    private static string StateText(TickByTickNative.Status status)
    {
        if (status.Running == 0)
        {
            return "Idle";
    ...
    {
        var count = TickByTickNative.TBT_GetPidCount();
        var nextPids = new List<(uint Pid, string Format, string LastPcm, bool IsSelected)>(count);
        for (var i = 0; i < count; i++)
        {
            if (TickByTickNative.TBT_GetPidInfo(i, out var info) != 0)
            {
                continue;
            }
    ...
    private void DrainNativeLog()
    {
        ...
        {
            buffer.Clear();
            var result = TickByTickNative.TBT_DrainLog(buffer, buffer.Capacity);
            if (result <= 0)
            {
                break;
            }
    ...
    private string ClockSourceStatusText(int activeClockSourceIndex)
    {
        if (activeClockSourceIndex >= 0)
        {
            return !clockSourceNames.TryGetValue(activeClockSourceIndex, out var activeSource)
                ? $"Clock source {activeClockSourceIndex} (active)"
                : $"{activeSource} (active)";
        }
        ...
        return clockSourceAvailabilityText;
    }
