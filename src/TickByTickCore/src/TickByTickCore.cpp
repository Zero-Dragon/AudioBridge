...
constexpr int32_t kDefaultMaxBufferAdvanceMs = 200;
...
public:
    IpcSecurityAttributes() {
        ...
        // packaged-application group access to objects created by TickByTick.
...
private:
    ...
    bool RefreshEndpoints(std::wstring* outError) {
        ...
        for (UINT index = 0; index < count; ++index) {
            ...
            // captures sessions that existed before TickByTick started.
...
void NativeDiagnostic(const wchar_t* message) {
    ...
    *(slash + 1) = L'\0';
    if (std::wcslen(path) + std::wcslen(L"TickByTickCore.native.log") + 1 >= std::size(path)) {
        return;
    }
    wcscat_s(path, L"TickByTickCore.native.log");
...
public:
    ...
    int32_t GetClockSourceInfo(int32_t position, TBT_ClockSourceInfo* info) const;
    ...
    int32_t GetPidInfo(int32_t index, TBT_PidInfo* info) const;
    int32_t GetStatus(TBT_Status* status) const;
...
private:
    ...
    std::vector<TBT_PidInfo> SnapshotPidsLocked() const;
...
TickByTickCore::TickByTickCore() {
    NativeDiagnostic(L"TickByTickCore constructor enter.");
    NativeDiagnostic(L"TickByTickCore constructor success.");
}

TickByTickCore::~TickByTickCore() {
    NativeDiagnostic(L"TickByTickCore destructor enter.");
    Shutdown();
    NativeDiagnostic(L"TickByTickCore destructor success.");
}

bool TickByTickCore::Initialize() {
    NativeDiagnostic(L"TickByTickCore::Initialize enter.");
    std::lock_guard<std::mutex> lock(stateMutex_);
    NativeDiagnostic(L"TickByTickCore::Initialize lock acquired.");
    SetLastErrorLocked(L"OK");
    NativeDiagnostic(L"TickByTickCore::Initialize success.");
    return true;
}

void TickByTickCore::Shutdown() {
    Stop();
}

int32_t TickByTickCore::StartTarget(const wchar_t* exePath,
                                     const wchar_t* hookDllPath,
                                     const wchar_t* outputDeviceId,
                                     std::int32_t prebufferMs,
                                     std::int32_t asioBufferFrames,
    ...
    if (!existingTargetPids.empty()) {
        ...
        SetLastErrorLocked(
                ...
                L"). Exit every instance, then open it again through Tick By Tick so the "
    ...
    try {
        feedbackThread_ = std::thread(&TickByTickCore::FeedbackThread, this);
...
int32_t TickByTickCore::SetOutputDevice(const wchar_t* outputDeviceId) {
    WAVEFORMATEXTENSIBLE format{};
    std::uint32_t pid = 0;
    {
        std::lock_guard<std::mutex> lock(stateMutex_);
...
int32_t TickByTickCore::SetPrebufferMs(std::int32_t prebufferMs) {
...
int32_t TickByTickCore::GetPrebufferMs() const {
    std::lock_guard<std::mutex> lock(stateMutex_);
    return prebufferMs_;
}

int32_t TickByTickCore::SetMaxBufferAdvanceMs(std::int32_t maxBufferAdvanceMs) {
...
int32_t TickByTickCore::GetMaxBufferAdvanceMs() const {
    std::lock_guard<std::mutex> lock(stateMutex_);
    return maxBufferAdvanceMs_;
}

int32_t TickByTickCore::SelectAudioPid(std::uint32_t pid) {
    if (pid == 0) {
        std::lock_guard<std::mutex> lock(stateMutex_);
...
int32_t TickByTickCore::RefreshDevices() {
...
int32_t TickByTickCore::GetDeviceCount() const {
    std::lock_guard<std::mutex> lock(stateMutex_);
    return static_cast<int32_t>(devices_.size());
}

int32_t TickByTickCore::GetDefaultDeviceIndex() const {
    std::lock_guard<std::mutex> lock(stateMutex_);
    return defaultDeviceIndex_;
}

int32_t TickByTickCore::GetDeviceId(int32_t index, wchar_t* buffer, int32_t bufferChars) const {
    std::lock_guard<std::mutex> lock(stateMutex_);
...
int32_t TickByTickCore::GetDeviceName(int32_t index, wchar_t* buffer, int32_t bufferChars) const {
    std::lock_guard<std::mutex> lock(stateMutex_);
    if (index < 0 || index >= static_cast<int32_t>(devices_.size())) {
        return kInvalidArgument;
    }
...
int32_t TickByTickCore::RefreshClockSources(const wchar_t* outputDeviceId) {
    ...
    std::lock_guard<std::mutex> routingLock(rendererRoutingMutex_);
    if (running_.load(std::memory_order_acquire) || renderer_.IsRunning()) {
        std::lock_guard<std::mutex> lock(stateMutex_);
        SetLastErrorLocked(L"Stop the current target before querying ASIO clock sources.");
        return kError;
    }
...
int32_t TickByTickCore::GetClockSourceCount() const {
    std::lock_guard<std::mutex> lock(stateMutex_);
    return static_cast<int32_t>(clockSources_.size());
}

int32_t TickByTickCore::GetClockSourceInfo(int32_t position,
                                            TBT_ClockSourceInfo* info) const {
    if (info == nullptr) {
        return kInvalidArgument;
    }
...
int32_t TickByTickCore::GetClockSourceName(int32_t position,
                                            wchar_t* buffer,
                                            int32_t bufferChars) const {
    std::lock_guard<std::mutex> lock(stateMutex_);
    if (position < 0 || position >= static_cast<int32_t>(clockSources_.size())) {
...
int32_t TickByTickCore::SetClockSource(std::int32_t clockSourceIndex) {
...
int32_t TickByTickCore::GetClockSource() const {
    std::lock_guard<std::mutex> lock(stateMutex_);
    return clockSourceIndex_;
}

int32_t TickByTickCore::GetPidCount() const {
    std::lock_guard<std::mutex> lock(stateMutex_);
    return static_cast<int32_t>(audioPids_.size());
}

int32_t TickByTickCore::GetPidInfo(int32_t index, TBT_PidInfo* info) const {
    if (info == nullptr) {
        return kInvalidArgument;
    }
    std::lock_guard<std::mutex> lock(stateMutex_);
...
int32_t TickByTickCore::GetStatus(TBT_Status* status) const {
    if (status == nullptr) {
        return kInvalidArgument;
    }
...
int32_t TickByTickCore::DrainLog(wchar_t* buffer, int32_t bufferChars) {
    if (buffer == nullptr || bufferChars <= 0) {
        return kInvalidArgument;
    }
    std::lock_guard<std::mutex> lock(stateMutex_);
...
int32_t TickByTickCore::GetLastError(wchar_t* buffer, int32_t bufferChars) const {
    std::lock_guard<std::mutex> lock(stateMutex_);
    return CopyWideString(lastError_, buffer, bufferChars);
}

void TickByTickCore::SetLastErrorLocked(const std::wstring& error) {
    lastError_ = error.empty() ? L"OK" : error;
}

void TickByTickCore::AppendLogTextLocked(const std::wstring& text) {
    logBuffer_ += text;
...
void TickByTickCore::Log(const wchar_t* format, ...) {
...
void TickByTickCore::LogDetectedStream(std::uint32_t pid,
                                        std::uint64_t streamId,
                                        const WAVEFORMATEX& format) {
...
void TickByTickCore::InitializeControlState() {
    if (control_ == nullptr) {
        return;
    }
...
void TickByTickCore::WriteControlState(std::uint32_t audioPid, bool finish) {
    if (control_ == nullptr) {
        return;
    }
...
void TickByTickCore::PublishRendererRoute(
        std::uint64_t streamId,
        std::uint32_t sampleRate,
        std::uint64_t consumedBaseline,
        std::uint64_t consumedOffset,
...
void TickByTickCore::PublishRendererCounters() {
    if (control_ == nullptr) {
        return;
    }
    std::lock_guard<std::mutex> controlLock(controlStateMutex_);
...
void TickByTickCore::LatchPipelineFaultLocked(const std::wstring& error) {
    bool expected = false;
...
bool TickByTickCore::WaitForCapturedDrainLocked(std::wstring* outError) {
...
bool TickByTickCore::StartPipeServer(std::wstring* outError) {
    try {
        pipeThread_ = std::thread(&TickByTickCore::PipeServerThread, this);
        return true;
    } catch (const std::exception& ex) {
        if (outError != nullptr) {
...
void TickByTickCore::PipeServerThread() {
    IpcSecurityAttributes ipcSecurity;
    while (running_.load()) {
        ...
        AddActivePipe(pipe);
        std::lock_guard<std::mutex> lock(clientThreadsMutex_);
        clientThreads_.emplace_back(&TickByTickCore::HandlePipeClient, this, pipe);
    }
}

void TickByTickCore::HandlePipeClient(HANDLE pipe) {
    for (;;) {
...
void TickByTickCore::HandlePipeMessage(DWORD type,
                                        DWORD pid,
...
void TickByTickCore::HandleFormatMessage(DWORD pid,
                                          const void* payload,
...
bool TickByTickCore::StartRendererForFormatLocked(
        std::uint32_t pid,
        const WAVEFORMATEXTENSIBLE& format) {
    if (pipelineFaulted_.load(std::memory_order_acquire)) {
        std::lock_guard<std::mutex> lock(stateMutex_);
    ...
    if (!error.empty()) {
        Log(L"ASIO clock warning: %s", error.c_str());
    }
...
void TickByTickCore::StartProcessMonitor() {
    if (processMonitorThread_.joinable()) {
        processMonitorThread_.join();
    }
    processMonitorThread_ = std::thread(&TickByTickCore::ProcessMonitorThread, this);
}

void TickByTickCore::ProcessMonitorThread() {
...
bool TickByTickCore::InjectProcessByPid(DWORD pid, std::wstring* outError) {
...
bool TickByTickCore::WaitForHookReady(HANDLE readyEvent,
                                       DWORD pid,
                                       std::wstring* outError) {
    ...
    if (outError != nullptr) {
        if (waitResult == WAIT_TIMEOUT) {
            *outError = L"Tick By Tick hook initialization timed out for PID " +
...
bool TickByTickCore::InjectDll(HANDLE process,
                                const std::filesystem::path& dllPath,
                                std::wstring* outError) {
...
bool TickByTickCore::InjectDllWithHelper(DWORD pid,
                                          const std::filesystem::path& helperPath,
                                          const std::filesystem::path& dllPath,
                                          std::wstring* outError) {
    ...
    if (!created) {
        if (outError != nullptr) {
            *outError = Win32Message(L"CreateProcessW(TickByTickInject32)");
        }
        return false;
    }
    ...
    if (exitCode != 0) {
        if (outError != nullptr) {
            ...
            std::swprintf(message,
                          std::size(message),
                          L"TickByTickInject32 failed with exit code %lu.",
...
bool TickByTickCore::CheckArchitectureMatch(HANDLE process, std::wstring* outError) {
...
std::filesystem::path TickByTickCore::DefaultHookDllPath(USHORT targetMachine) const {
    HMODULE module = nullptr;
    GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
                               GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                       reinterpret_cast<LPCWSTR>(&TBT_StartTarget),
                       &module);
    wchar_t path[MAX_PATH]{};
    const wchar_t* dllName =
            targetMachine == IMAGE_FILE_MACHINE_I386 ? L"TickByTickHook32.dll"
                                                     : L"TickByTickHook.dll";
    if (module != nullptr &&
...
std::filesystem::path TickByTickCore::DefaultInjectHelperPath() const {
    HMODULE module = nullptr;
    GetModuleHandleExW(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS |
                               GET_MODULE_HANDLE_EX_FLAG_UNCHANGED_REFCOUNT,
                       reinterpret_cast<LPCWSTR>(&TBT_StartTarget),
    ...
    if (module != nullptr &&
        GetModuleFileNameW(module, path, static_cast<DWORD>(std::size(path))) > 0) {
        return std::filesystem::path(path).parent_path() / L"TickByTickInject32.exe";
    }
    return L"TickByTickInject32.exe";
}

std::vector<TBT_PidInfo> TickByTickCore::SnapshotPidsLocked() const {
    std::vector<TBT_PidInfo> result;
...
TickByTickCore* g_core = nullptr;
SRWLOCK g_coreLock = SRWLOCK_INIT;

TickByTickCore* Core() noexcept {
    if (g_core != nullptr) {
        return g_core;
    }
    ...
    if (g_core == nullptr) {
        NativeDiagnostic(L"Core() allocating TickByTickCore.");
        try {
            g_core = new (std::nothrow) TickByTickCore();
        } catch (...) {
...
int32_t CopyStartupError(const wchar_t* message, wchar_t* buffer, int32_t bufferChars) {
    return CopyWideString(message != nullptr ? std::wstring(message) : std::wstring(L"TickByTickCore is not initialized."),
                          buffer,
                          bufferChars);
}
...
TBT_API int32_t TBT_CALL TBT_Initialize(void) {
    tickbytick::NativeDiagnostic(L"TBT_Initialize enter.");
    __try {
        auto* core = tickbytick::Core();
        if (core == nullptr) {
            tickbytick::NativeDiagnostic(L"TBT_Initialize failed: TickByTickCore allocation returned null.");
            return -1;
        }
        const int32_t result = core->Initialize() ? 0 : -1;
        tickbytick::NativeDiagnostic(result == 0 ? L"TBT_Initialize success." : L"TBT_Initialize returned failure.");
        return result;
    } __except (EXCEPTION_EXECUTE_HANDLER) {
        tickbytick::NativeDiagnostic(L"TBT_Initialize caught a structured exception.");
        return -1;
    }
}

TBT_API void TBT_CALL TBT_Shutdown(void) {
    if (auto* core = tickbytick::Core()) {
        core->Shutdown();
    }
}
...
TBT_API int32_t TBT_CALL TBT_StartTarget(const wchar_t* exePath,
    ...
    return core->StartTarget(
            ...
            outputDeviceId,
            core->GetPrebufferMs(),
            tickbytick::kDefaultAsioBufferFrames,
            false);
}
...
TBT_API int32_t TBT_CALL TBT_GetPrebufferMs(void) {
    auto* core = tickbytick::Core();
    return core != nullptr ? core->GetPrebufferMs() : tickbytick::kDefaultPrebufferMs;
}
...
TBT_API int32_t TBT_CALL TBT_GetMaxBufferAdvanceMs(void) {
    ...
    return core != nullptr
            ...
            : tickbytick::kDefaultMaxBufferAdvanceMs;
}
...
TBT_API int32_t TBT_CALL TBT_GetDeviceId(int32_t index, wchar_t* buffer, int32_t bufferChars) {
    ...
    return core != nullptr
            ...
            : tickbytick::CopyStartupError(L"TickByTickCore is not initialized.", buffer, bufferChars);
}
...
TBT_API int32_t TBT_CALL TBT_GetClockSourceInfo(int32_t position,
                                                TBT_ClockSourceInfo* info) {
    auto* core = tickbytick::Core();
...
TBT_API int32_t TBT_CALL TBT_GetClockSourceName(int32_t position,
    ...
    return core != nullptr
            ...
            : tickbytick::CopyStartupError(
                      L"TickByTickCore is not initialized.", buffer, bufferChars);
}
