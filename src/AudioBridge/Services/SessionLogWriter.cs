...
internal sealed class SessionLogWriter : IDisposable
{
    ...
    private SessionLogWriter(string? filePath)
    {
        ...
        File.WriteAllText(
            filePath,
            $"[{DateTime.Now:HH:mm:ss.fff}] AudioBridge session log. " +
        ...
        {
            IsBackground = true,
            Name = "AudioBridge session log writer"
        };
        writerThread.Start();
    }
    ...
    public static SessionLogWriter Create()
    {
        foreach (var directory in CandidateDirectories())
        {
            try
            {
                ...
                var fileName = $"AudioBridge-{timestamp}-p{Environment.ProcessId}.log";
                return new SessionLogWriter(Path.Combine(directory, fileName));
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"AudioBridge log path unavailable: {directory}: {ex.Message}");
            }
        }
    ...
    public static void WriteCrashLog(Exception exception)
    {
        foreach (var directory in CandidateDirectories())
        {
            try
            {
                Directory.CreateDirectory(directory);
                var timestamp = DateTime.Now.ToString("yyyyMMdd-HHmmss-fff");
                var fileName = $"AudioBridge-crash-{timestamp}-p{Environment.ProcessId}.log";
                File.WriteAllText(
            ...
            {
                Debug.WriteLine($"AudioBridge crash log path unavailable: {directory}: {ex.Message}");
            }
        }
    }
    ...
    private void WriteLoop()
    {
        ...
        while (!pendingWrites.IsCompleted)
        {
            ...
            {
                Debug.WriteLine($"AudioBridge log write failed: {ex.Message}");
            }
        }
    }
    ...
    private static IEnumerable<string> CandidateDirectories()
    {
        ...
        return new[]
        {
            ...
            string.IsNullOrWhiteSpace(localAppData)
                ? null
                : Path.Combine(localAppData, "AudioBridge", "logs"),
            Path.Combine(Path.GetTempPath(), "AudioBridge", "logs")
        }
